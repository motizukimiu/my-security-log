...
# --- 0. セキュリティ対策：Secretsから情報を取得 ---
try:
    SPREADSHEET_URL = st.secrets["spreadsheet_url"]
    AUTH_KEY = st.secrets["auth_key"]
except KeyError:
    st.error("Secretsの設定（spreadsheet_url, auth_key）が見つかりません。")
    st.stop()

# ページタイトルの変更
st.set_page_config(page_title="学習管理", layout="wide")
...
# --- 4. メインコンテンツ ---
    ...
    elif auth_status is False: st.error("IDまたはパスワードが間違っています")
    else: st.warning("左メニューからログインしてください")